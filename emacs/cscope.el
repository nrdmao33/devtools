...
;;; Jim Murphy Modification
;;; Add support for a stack which will remember the buffers from whence
;;; queries were made and then allow you to pop back to those files.
;;; this allows one to follow a long list of function calls and then
;;; to retrace your steps. After retracing  your steps you can replay
;;; the forward progression of the original query sequence by following
;;; a forward stack as well.

(defvar stack-cscope-back-stack nil
  "Stack of places from whence a query was made")

(defvar stack-cscope-forward-stack nil
  "Stack of places popped to - so we can replay an excursion.
This stack is cleared when a new cscope query is made.")

(defun stack-cscope-push ()
  "Push the current buffer and line number onto the stack
so that we can come back here as we unwind our walk through
the source code. Clear the forward stack"
  (interactive)
  (setq new-item (list (buffer-name) (point)))
  (setq stack-cscope-back-stack (cons new-item stack-cscope-back-stack))
  (setq stack-cscope-forward-stack nil))

(defun stack-cscope-pop ()
  "Pop the top of stack-cscope-back-stack and goto that buffer and line
also remember this location in the forward stack"
  (interactive)
  (let ((new-location) (new-filename) (new-point))
    (if stack-cscope-back-stack
	(progn
	  (setq new-location (car stack-cscope-back-stack))
	  (setq stack-cscope-back-stack (cdr stack-cscope-back-stack))
	  (setq new-filename (car new-location))
	  (setq new-point (car (cdr new-location)))
	  (switch-to-buffer new-filename)
	  (goto-char new-point)
	  (setq stack-cscope-forward-stack
		(cons new-location stack-cscope-forward-stack)))
      (error "CSCOPE: End of backtrace"))))

(defun stack-cscope-forward ()
  "Move forward to the next item on the stack-cscope-forward-stack"
  (interactive)
  (let ((new-location) (new-filename) (new-point))
    (if stack-cscope-forward-stack
	(progn
	  (setq new-location (car stack-cscope-forward-stack))
	  (setq stack-cscope-forward-stack (cdr stack-cscope-forward-stack))
	  (setq new-filename (car new-location))
	  (setq new-point (car (cdr new-location)))
	  (switch-to-buffer new-filename)
	  (goto-char new-point)
	  (setq stack-cscope-back-stack
		(cons new-location stack-cscope-back-stack)))
      (error "CSCOPE: End of forward excursion"))))

;;; End Jim Murphy modification

(defvar cscope-bindings-2deep t
...
(defvar cscope-bindings-3deep nil
...
\"cscope-bindings-2deep\"."
  ...
  (local-set-key "\C-cb" 'stack-cscope-pop)
  (local-set-key "\C-cB" 'stack-cscope-pop)
  (local-set-key "\C-c\C-b" 'stack-cscope-pop)

  (local-set-key "\C-cn" 'stack-cscope-forward)
  (local-set-key "\C-cN" 'stack-cscope-forward)
  (local-set-key "\C-c\C-n" 'stack-cscope-forward)
)
...
\"cscope-bindings-3deep\"."
  ...
  (local-set-key "\C-c\C-sb" 'stack-cscope-pop)
  (local-set-key "\C-c\C-sB" 'stack-cscope-pop)
  (local-set-key "\C-c\C-s\C-b" 'stack-cscope-pop)

  (local-set-key "\C-c\C-sn" 'stack-cscope-forward)
  (local-set-key "\C-c\C-sN" 'stack-cscope-forward)
  (local-set-key "\C-c\C-s\C-n" 'stack-cscope-forward)
)
...
(if (not (boundp 'cscope-keymap))
	(progn
	  ...
	  (define-key cscope-keymap "b" 'stack-cscope-pop)
	  (define-key cscope-keymap "B" 'stack-cscope-pop)
	 
	  (define-key cscope-keymap "n" 'stack-cscope-forward)
	  (define-key cscope-keymap "N" 'stack-cscope-forward)
